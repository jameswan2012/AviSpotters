...
export async function POST(request: Request) {
  ...
  const maxBytes = ext === "gif" ? 15 * 1024 * 1024 : 1 * 1024 * 1024;
  if (file.size > maxBytes) {
    return NextResponse.json({ error: ext === "gif" ? "GIF 最大 15MB" : "頭像最大 1MB" }, { status: 400 });
  }

  const bytes = new Uint8Array(await file.arrayBuffer());
  try {
    assertMagicMatchesAllowed(bytes, ext === "png" ? ["png"] : ext === "gif" ? ["gif"] : ["jpeg"]);
    await scanWithClamAVIfEnabled(bytes, file.name || `avatar.${ext}`);
  } catch (e: any) {
  ...
  const rel = path.posix.join("users", user.id, `avatar.${ext}`);
  ...
  return NextResponse.json({ ok: true, avatarUrl: `/api/users/${encodeURIComponent(user.id)}/avatar?v=${now.getTime()}` });
}
