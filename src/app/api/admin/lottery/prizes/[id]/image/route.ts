...
export async function POST(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (file.size > 2 * 1024 * 1024) return NextResponse.json({ error: "圖片最大 2MB" }, { status: 400 });

  const bytes = new Uint8Array(await file.arrayBuffer());
  try {
    assertMagicMatchesAllowed(bytes, ext === "png" ? ["png"] : ["jpeg"]);
    await scanWithClamAVIfEnabled(bytes, file.name || `lottery-prize.${ext}`);
  } catch (e: any) {
  ...
  const rel = path.posix.join("lottery-prizes", id, `image.${ext}`);
  const abs = path.join(uploadsRoot(), rel);
  await writeFileEnsured(abs, bytes);

  const now = new Date();
  const prize = await prisma.lotteryPrize.update({
    where: { id },
    data: { imagePath: abs, imageMime: mime, imageSizeBytes: file.size, imageUpdatedAt: now },
    select: { id: true, rewardType: true, rewardItemId: true },
  });

  if (prize.rewardType === "custom" && prize.rewardItemId) {
    await prisma.rewardItem.update({
      where: { id: prize.rewardItemId },
      data: { imagePath: abs, imageMime: mime, imageSizeBytes: file.size, imageUpdatedAt: now },
      select: { id: true },
    });
  }

  return NextResponse.json({ ok: true, imageUrl: `/api/lottery/prizes/${encodeURIComponent(id)}/image?v=${now.getTime()}` });
}
