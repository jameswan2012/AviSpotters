...
import { setSiteFooterGroupQr } from "@/lib/site-settings";
...
export async function POST(request: Request) {
  ...
  if (file.size > 2 * 1024 * 1024) return NextResponse.json({ error: "QR 圖片最大 2MB" }, { status: 400 });

  const bytes = new Uint8Array(await file.arrayBuffer());
  try {
    assertMagicMatchesAllowed(bytes, ext === "png" ? ["png"] : ["jpeg"]);
    await scanWithClamAVIfEnabled(bytes, file.name || `group-qr.${ext}`);
  } catch (e: any) {
  ...
  const rel = path.posix.join("site", `group-qr.${ext}`);
  const abs = path.join(uploadsRoot(), rel);
  await writeFileEnsured(abs, bytes);

  const now = new Date();
  await setSiteFooterGroupQr({
    groupQrPath: abs,
    groupQrMime: mime,
    groupQrSizeBytes: file.size,
    groupQrUpdatedAt: now,
    updatedById: user.id,
  });
  return NextResponse.json({ ok: true, groupQrUrl: `/api/site/group-qr?v=${now.getTime()}` });
}

export async function DELETE() {
  const { user } = await requireSuperAdmin();
  await setSiteFooterGroupQr({ groupQrPath: null, groupQrMime: null, groupQrSizeBytes: null, groupQrUpdatedAt: null, updatedById: user.id });
